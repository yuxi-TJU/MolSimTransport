...
def create_and_run_for_field(base_dir, poscar_file, eefs, input_energy_range, input_energy_interval, save_mat_files): #, input_energy_number):
...
def merge_transmission_files(base_dir, electric_field_range):
    ...
    for idx, eefs in enumerate(electric_field_range):
...
def main():
    ...
    electric_field_range = np.array([0.0002])  # If only a single bias point is calculated, comment out the line above
    ...
    with ProcessPoolExecutor(max_workers=2) as executor:
        futures = [executor.submit(create_and_run_for_field, base_dir, poscar_file, eefs, input_energy_range, input_energy_interval, save_mat_files)
