...
def generate_mo_section(eigenenergies_file, eigenvectors_file, atom_order_file):
    ...
    """
    energies = np.loadtxt(eigenenergies_file, skiprows=1)
...
def generate_atoms_section(em_file):
    ...
    with open(em_file, 'r') as f:
        ...
        for atom_index, line in enumerate(lines, start=1):
            ...
            atomic_num = atom_to_number[element]
...
def generate_gto_section(json_file, atoms_file):
    ...
    with open(atoms_file, 'r') as f:
        for atomic_index, line in enumerate(f.readlines()[2:], start=1):
            parts = line.split()
            atomic_num = str(atom_to_number[parts[0]])
...
def main():
    ...
    em_file = input(">>> Enter EM XYZ file name (e.g., 3au-AQ.xyz): ")

    with open(em_file, 'r') as f:
        lines = [line for line in f if line.strip()]

    cleaned_em_file = 'cleaned_' + em_file
    with open(cleaned_em_file, 'w') as f:
        f.writelines(lines)

    atomic_orbitals_json_file = files('MolSimTransport.utils.atomic_data').joinpath('gfn1_gto_basis.json')
    output_file = 'MPSH.molden'
    generate_molden_file(cleaned_em_file, eigenenergies_file, eigenvectors_file, atomic_orbitals_json_file, output_file)
